...
#if SYZ_EXECUTOR || __NR_syz_net_down_sleep_up || __NR_syz_failure_net_down || \
    __NR_syz_failure_net_up
#include <stdlib.h>
...
static long syz_net_down_sleep_up(volatile long usec) {
  ...
  */
  system(
      "iptables -A INPUT -s 192.168.0.1 -j ACCEPT; iptables -A OUTPUT -d 192.168.0.1 -j ACCEPT; \
...
extern struct executeControl *execCtl;
#define SUPPORT_VMS 64
struct executeControl {
  unsigned char hasTestcase[SUPPORT_VMS];
  unsigned char coverEnabled[SUPPORT_VMS];
  unsigned char executionsFinish[SUPPORT_VMS];
  unsigned char tmpDirEstablished;
  uint64_t synchBit;
};
#if SYZ_EXECUTOR || __NR_syz_failure_send || __NR_syz_failure_recv
static long syz_failure_send(unsigned char idx) {
  execCtl->synchBit |= (uint64_t)1 << idx;
  return 0;
}
...
extern int *shm_need_reboot;
#if SYZ_EXECUTOR || __NR_syz_reboot || __NR_syz_failure_up ||                  \
    __NR_syz_failure_down
...
extern struct outputControl *output_ctl_pos;
...
const char kvm_ppc64_mr[] =
    "\x00\x00\xa0\x3c\x00\x00\xa5\x60\xc6\x07\xa5\x78\xad\x0b\xa5\x64\xde\xc0"
    "\xa5\x60\x78\x2b\xa4\x7c\x78\x23\x83\x7c";
const char kvm_ppc64_ld[] =
    "\x00\x00\xe0\x3d\x00\x00\xef\x61\xc6\x07\xef\x79\xad\x0b\xef\x65\xde\xc0"
    "\xef\x61\x00\x00\x20\x3f\x00\x00\x39\x63\xc6\x07\x39\x7b\x17\x00\x39\x67"
    "\xf8\xff\x39\x63\x00\x00\xf9\xf9\x00\x00\x79\xe8";
const char kvm_ppc64_recharge_dec[] =
    "\x00\x00\x80\x3e\x00\x00\x94\x62\xc6\x07\x94\x7a\xff\x07\x94\x66\xff\xff"
    "\x94\x62\xa6\x03\x96\x7e\x24\x00\x00\x4c";
...
static int wait_for_loop(int pid) {
  ...
  fprintf(stderr, "executionFinished %d\n", output_ctl_pos->executionFinished);
  return WEXITSTATUS(status);
}
#endif
...
static void push_back(Filter *filter_array, struct sock_filter filter) {
  if (filter_array->count == kFilterMaxSize)
    failmsg("can't add another syscall to seccomp filter", "count=%zu",
            filter_array->count);
...
static void ValidateArchitecture(Filter *f) {
  ...
  struct sock_filter filter2 =
      BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, PRIMARY_ARCH, 1, 0);
  push_back(f, filter1);
  push_back(f, filter2);
  Disallow(f);
}
