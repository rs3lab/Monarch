...
import (
	...
	"monarch/pkg/csource"
	"monarch/pkg/host"
	instancePkg "monarch/pkg/instance"
	"monarch/pkg/log"
...
func Run(crashLog []byte, cfg *mgrconfig.Config, features *host.Features, reporter *report.Reporter,
	...
	log.Logf(0, "----- repro Run staring repro vmindexes: %d", len(vmIndexes))
	...
	ctx := &context{
		...
		instances:    make([]*instance, len(vmIndexes)),
		...
		go func() {
			...
			for vmIndex := range ctx.bootRequests {
				...
				for try := 0; try < maxTry; try++ {
					...
					var err error
					inst, err = ctx.initInstance(cfg, vmPool, vmIndex+cfg.FuzzingVMs)
					if err != nil {
				...
				ctx.instances[vmIndex] = inst
				log.Logf(0, "----- repro instances len: %d, bootRequests: %d", len(ctx.instances), len(ctx.bootRequests))
			}
		}()
	}
	go func() {
	...
	defer func() {
		...
		for i := 0; i < len(ctx.instances); {
			if ctx.instances[i] != nil {
				ctx.instances[i].Close()
				i++
			}
		}
	...
	if res != nil {
		...
		for attempts := 0; ctx.report.Corrupted && attempts < 3; attempts++ {
			...
			if res.CRepro {
				_, err = ctx.testCProg(res.Progs, res.Duration, res.Opts)
			} else {
				_, err = ctx.testProg(res.Progs, res.Duration, res.Opts)
			}
			if err != nil {
				return nil, nil, err
			}
...
func (ctx *context) repro(entries []*prog.LogEntry, crashStart int) (*Result, error) {
	...
	log.Logf(0, "----- repro vmindex: %d", len(ctx.instances))
...
func (ctx *context) extractProg(entries []*prog.LogEntry) (*Result, error) {
	...
	for _, timeout := range ctx.testTimeouts {
		...
		if res != nil {
			callNum := 0
			for _, prog := range res.Progs {
				callNum += len(prog.Calls)
			}
			ctx.reproLogf(3, "found reproducer with %d syscalls", callNum)
			return res, nil
		}
...
func (ctx *context) extractProgSingle(entries []*prog.LogEntry, duration time.Duration) (*Result, error) {
	...
	for _, ent := range entries {
		crashed, err := ctx.testProg(ent.Ps, duration, opts)
		if err != nil {
			return nil, err
		}
		if crashed {
...
func (ctx *context) extractProgBisect(entries []*prog.LogEntry, baseDuration time.Duration) (*Result, error) {
	...
	for i, entry := range entries {
		for j, P := range entry.Ps {
			...
			progs[j].Calls = append(progs[j].Calls, P.Calls...)
		}
	}

	/*
			prog := &prog.Prog{
				Target: entries[0].Ps[0].Target,
			}
			for _, entry := range entries {
		        for _, P := range entry.Ps {
				    prog.Calls = append(prog.Calls, P.Calls...)
		        }
			}
	*/

	dur := duration(len(entries)) * 3 / 2
	crashed, err := ctx.testProg(progs, dur, opts)
	if err != nil {
		return nil, err
	}
	if crashed {
...
func (ctx *context) minimizeProg(res *Result) (*Result, error) {
	...
	//TODO: modify to correct args
	res.Progs, _ = prog.Minimize(res.Progs, -1, -1, true, -1,
...
func (ctx *context) simplifyProg(res *Result) (*Result, error) {
	...
	for _, simplify := range progSimplifies {
		...
		crashed, err := ctx.testProg(res.Progs, res.Duration, opts)
		if err != nil {
			return nil, err
		}
		if !crashed {
...
func (ctx *context) extractC(res *Result) (*Result, error) {
	...
	crashed, err := ctx.testCProg(res.Progs, res.Duration, res.Opts)
	if err != nil {
		return nil, err
	}
	res.CRepro = crashed
...
func (ctx *context) simplifyC(res *Result) (*Result, error) {
	...
	for _, simplify := range cSimplifies {
		...
		crashed, err := ctx.testCProg(res.Progs, res.Duration, opts)
		if err != nil {
			return nil, err
		}
		if !crashed {
...
func (ctx *context) testProg(ps []*prog.Prog, duration time.Duration, opts csource.Options) (crashed bool, err error) {
	entry := prog.LogEntry{Ps: ps}
...
func (ctx *context) testProgs(entries []*prog.LogEntry, duration time.Duration, opts csource.Options) (
	...
	//for ; len(ctx.instances) < ctx.cfg.FuzzingVMs; {}

	//inst := <-ctx.instances
	var insts []*instance
	//instNum := len(ctx.instances)

	//log.Logf(0, "----- repro testProgs ctx.instances: %d", len(ctx.instances))

	for i := 0; i < ctx.cfg.FuzzingVMs; {
		if ctx.instances[i] != nil {
	...
	vmProgFile, err := insts[0].Copy(progFile)
	if err != nil {
		return false, fmt.Errorf("failed to copy to VM: %v", err)
	}

	program := ""
	for idx, P := range entries[0].Ps {
		program += P.String()
		if idx != len(entries[0].Ps)-1 {
			program += "---\n"
		}
	}
	...
	command := instancePkg.ExecprogCmd(insts[0].execprogBin, insts[0].executorBin,
	...
	for _, inst := range insts {
		instExecutorCmd, _, err := inst.GenExecutorCmd(command)
		if err != nil {
			return false, err
		}
	...
	return ctx.testImpl(insts[0].Instance, command+"|"+instExecutorCmds, duration)
}

func (ctx *context) testCProg(ps []*prog.Prog, duration time.Duration, opts csource.Options) (crashed bool, err error) {
	...
	crashed, err = ctx.testBin(bins, duration)
	if err != nil {
		return false, err
	}
	return crashed, nil
...
func (ctx *context) testBin(bins []string, duration time.Duration) (crashed bool, err error) {
	//inst := <-ctx.instances
	cmd := ""
	//instNum := len(ctx.instances)
	var firstInst *vm.Instance
	for i := 0; i < ctx.cfg.FuzzingVMs; {
		if ctx.instances[i] == nil {
			continue
		}
		...
		defer ctx.returnInstance(inst)
		if i == 0 {
			firstInst = inst.Instance
		}

		bin, err := inst.Copy(bins[i])
		if err != nil {
			return false, fmt.Errorf("failed to copy to VM: %v", err)
		}
		cmd += strings.Join(inst.GetSSHArgs(bin), " ") + ";"
		i++
	}
	return ctx.testImpl(firstInst, cmd, duration)
}

func (ctx *context) testImpl(inst *vm.Instance, command string, duration time.Duration) (crashed bool, err error) {
	outc, errc, err := inst.Run(duration, nil, command, nil)//tao modified
	if err != nil {
