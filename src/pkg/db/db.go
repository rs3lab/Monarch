...
func (db *DB) Save(key string, val [][]byte, seq uint64) {
	...
	if rec, ok := db.Records[key]; ok && seq == rec.Seq && bytes.Equal(custom.Concat(val...), custom.Concat(rec.Val...)) {
		return
	}
...
func serializeRecord(w *bytes.Buffer, key string, val [][]byte, seq uint64) {
	...
	if len(val) == 0 {
		...
		for i := uint32(0); i < uint32(len(val)); i++ {
			...
			if _, err := fw.Write(val[i]); err != nil {
				panic(err)
			}
			fw.Close()
...
func deserializeRecord(r *bufio.Reader) (key string, val [][]byte, seq uint64, err error) {
	...
	//the number of concurrent sub-testcases in one testcase
	var subTestcaseNum uint32
	if err = binary.Read(r, binary.LittleEndian, &subTestcaseNum); err != nil {
		return
	}

	for i := uint32(0); i < subTestcaseNum; i++ {
		var valLen uint32
		...
		if valLen != 0 {
			...
			var valTmp []byte
			if valTmp, err = ioutil.ReadAll(fr); err != nil {
				return
			}
			val = append(val, valTmp)
			fr.Close()
		}
	}
	return
