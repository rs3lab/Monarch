...
func (ctx *linux) Parse(output []byte) *Report {
	...
	for questionable := false; ; questionable = true {
		...
		//if title == "WARNING in kcov_task_exit" {
		if title == "" {
			prefix = nil
...
var linuxOopses = append([]*oops{
	{
		...
		[]oopsFormat{
			{
				title:  compile("ERROR:"),
				report: compile("ERROR: (.*?)Sanitizer: (.*)"),
				fmt:    "ASAN/LSAN: %[1]vSanitizer: %[2]v",
				//report:	compile("ERROR: (.*?)Sanitizer: (.*?)"),
				/*
				   fmt:    "KASAN: %[1]v",
				   stack:  &stackFmt{
			...
			{
				title:  compile("WARNING"),
				report: compile("WARNING: (.*?)Sanitizer: (.*)"),
				fmt:    "TSAN/MSAN/CSAN: %[1]vSanitizer: $[2]v",
			},
		},
		[]*regexp.Regexp{},
	},
