...
func (mgr *Manager) collectStats() []UIStat {
	...
	stats := []UIStat{
		...
		{Name: "client signal", Value: fmt.Sprint(rawStats["client signal"])},
		{Name: "server signal", Value: fmt.Sprint(rawStats["server signal"])},
		{Name: "client coverage", Value: fmt.Sprint(rawStats["client coverage"]), Link: "/cover"},
		{Name: "server coverage", Value: fmt.Sprint(rawStats["server coverage"]), Link: "/cover"},
	}
	...
	delete(rawStats, "client signal")
	delete(rawStats, "server signal")
	delete(rawStats, "client coverage")
	delete(rawStats, "server coverage")
...
func (mgr *Manager) httpCorpus(w http.ResponseWriter, r *http.Request) {
	...
	for sig, inp := range mgr.corpus {
		...
		ps, err := mgr.target.Deserialize(inp.Prog, prog.NonStrict)
		if err != nil {
		...
		psStr = strings.Replace(psStr, "syz_failure_recv-syz_failure_net_down-syz_failure_send", "", -1)
	    psStr = strings.Replace(psStr, "syz_failure_recv-syz_failure_net_up-syz_failure_send", "", -1)
	    psStr = strings.Replace(psStr, "syz_failure_recv-syz_failure_down-syz_failure_send", "", -1)
	    psStr = strings.Replace(psStr, "syz_failure_recv-syz_failure_up-syz_failure_send", "", -1)

	    re := regexp.MustCompile(`syz_failure_[a-z]*`)
	    psStr = re.ReplaceAllString(psStr, "F")

		data.Inputs = append(data.Inputs, &UIInput{
			Sig:   sig,
			Short: psStr,
			Cover: len(inp.CliCover) + len(inp.SrvCover),
		})
	}
...
func (mgr *Manager) httpCoverCover(w http.ResponseWriter, r *http.Request, funcFlag int, isHTMLCover bool) {
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("getReportGenerator(): failed to generate coverage profile: %v", err), http.StatusInternalServerError)
		return
	}
	...
	if sig := r.FormValue("input"); sig != "" {
		inp := mgr.corpus[sig]
		tmpPCs := coverToPCs(rg, inp.CliCover)
		tmpPCs = append(tmpPCs, coverToPCs(rg, inp.SrvCover)...)
		progs = append(progs, cover.Prog{
			Data: string(custom.Concat(inp.Prog...)),
			PCs:  tmpPCs,
		})
	} else {
		call := r.FormValue("call")
	...
	if funcFlag == DoRawCoverFiles {
		if err := rg.DoRawCoverFiles(w, progs, coverFilter); err != nil {
			http.Error(w, fmt.Sprintf("DoRawCoverFiles(): failed to generate coverage profile: %v", err), http.StatusInternalServerError)
			return
		}
		runtime.GC()
		return
	...
	if err := do(w, progs, coverFilter); err != nil {
		http.Error(w, fmt.Sprintf("do(): failed to generate coverage profile: %v", err), http.StatusInternalServerError)
		return
	}
	runtime.GC()
}
...
func (mgr *Manager) httpCoverFallback(w http.ResponseWriter, r *http.Request) {
	...
	for _, inp := range mgr.corpus {
		maxSignal.Merge(inp.CliSignal.Deserialize())
		maxSignal.Merge(inp.SrvSignal.Deserialize())
	}
...
func (mgr *Manager) httpPrio(w http.ResponseWriter, r *http.Request) {
	...
	for _, inp := range mgr.corpus {
		...
		/*
			        var tmp []*prog.Prog
			        for _, progData := range inp.Prog {
					    p, err := mgr.target.Deserialize(progData, prog.NonStrict)
					    if err != nil {
						    http.Error(w, fmt.Sprintf("failed to deserialize program: %v", err), http.StatusInternalServerError)
						    return
					    }
...
func (mgr *Manager) httpInput(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range inp.Prog {
		progsStr = append(progsStr, p...)
		progsStr = append(progsStr, []byte("---\n")...)
	}
	w.Write(progsStr)
	//w.Write(custom.Concat(inp.Prog...))
}
