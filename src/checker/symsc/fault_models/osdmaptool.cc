...
int main(int argc, const char **argv)
{
  ...
  std::vector<int> marked_downs, marked_outs;
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
	...
	exit(EXIT_FAILURE);
      ...
      marked_outs.push_back(marked_out);
    } else if (ceph_argparse_witharg(args, i, &val, "--mark-down", (char*)NULL)) {
      marked_downs.push_back(std::stoi(val));
  ...
  for(long unsigned int i = 0; i < marked_downs.size(); i++) {
    int id = marked_downs[i];
  ...
  for(long unsigned int i = 0; i < marked_outs.size(); i++) {
    int id = marked_outs[i];
    if (id >=0 && id < osdmap.get_max_osd()) {
