...
func main() {
	...
	*/
	...
	for id, syscall := range target.Syscalls {
		switch syscall.Name {
			case "syz_failure_sync":		fuzzer.sCalls.SyncfailId = id
			case "syz_failure_recv":		fuzzer.sCalls.RecvId = id
			case "syz_failure_send":		fuzzer.sCalls.SendId = id
			case "syz_failure_up":			fuzzer.sCalls.UpId = id
			case "syz_failure_down":		fuzzer.sCalls.DownId = id
			case "syz_failure_net_up":		fuzzer.sCalls.NetUpId = id
			case "syz_failure_net_down":	fuzzer.sCalls.NetDownId = id
			case "sync":					fuzzer.sCalls.SyncId = id
			case "fsync":					fuzzer.sCalls.FsyncId = id
			case "fdatasync":				fuzzer.sCalls.FdatasyncId = id
			case "syz_failure_crash_client":		fuzzer.sCalls.CrashClient = id
			case "syz_failure_crash_server":		fuzzer.sCalls.CrashServer = id
		}
	}
...
func (fuzzer *Fuzzer) addInputFromAnotherFuzzer(inp rpctype.RPCInput) {
	...
	fuzzer.addInputToCorpus(p, cliSign, srvSign, sig)
}
...
func (fuzzer *Fuzzer) addInputToCorpus(ps []*prog.Prog, cliSign signal.Signal, srvSign signal.Signal, sig hash.Sig) {
