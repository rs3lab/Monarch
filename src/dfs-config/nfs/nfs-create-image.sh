...
if [ $PERF = "true" ]; then
    ...
    sudo chroot $DIR /bin/bash -c "apt-get update; apt-get install -y flex bison python-dev libelf-dev libunwind8-dev libaudit-dev libslang2-dev libperl-dev binutils-dev liblzma-dev libnuma-dev nfs-kernel-server vim"
...
sudo chroot $DIR /bin/bash -c "apt-get update; apt-get install -y vim nfs-kernel-server"
...
sudo sed -i 's/auto/allow-hotplug/' $DIR/etc/network/interfaces
...
sudo umount /mnt/$DIR
sudo qemu-img convert -f raw -O qcow2 ./stretch.img ./stretch.qcow2
