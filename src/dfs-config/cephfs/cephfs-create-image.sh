...
DEBOOTSTRAP_PARAMS="--arch=$DEBARCH --include=$PREINSTALL_PKGS --components=main $RELEASE $DIR"
if [ $FOREIGN = "true" ]; then
...
echo "dfs-fuzzing" | sudo tee $DIR/etc/hostname
ssh-keygen -f $RELEASE.id_rsa -t rsa -N ''
sudo mkdir -p $DIR/root/.ssh/
cat $RELEASE.id_rsa.pub | sudo tee $DIR/root/.ssh/authorized_keys

sudo chroot $DIR /bin/bash -c "apt-get install -y vim"
# ./install-deps.sh"
...
#copy compiled Ceph into chroot
#curDir=$PWD
#cd /home/tlyu/dfs-fuzzing/dfs-compilation/ceph/build
#sudo make install DESTDIR=$curDir/$DIR
#cd $curDir

# Build a disk image
dd if=/dev/zero of=$RELEASE.img bs=1M seek=$SEEK count=40960
