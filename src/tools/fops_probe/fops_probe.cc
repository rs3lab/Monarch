...
int main(int argc, char **argv) {
  ...
  uint64_t *cover = (uint64_t *)mmap(
      NULL, COVER_SIZE * 8, PROT_READ | PROT_WRITE, MAP_SHARED, kcov, 0);
  if (cover == MAP_FAILED)
  ...
  probe_callback(cover, kallsyms, "do_mmap",
...
bool should_skip(const std::string &sym) {
  static const char *skip[] = {
      "security", "tomoyo", "selinux",     "apparmor",
      "smack",    "policy", "stack_trace", "should_fail",
      "debug",    "trace",  "snprintf",    "vsnprintf",
  };
