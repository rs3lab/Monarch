...
// KCOVFUZZER=trace_filter ./fuzzer -max_len=100 -only_ascii=1
// corpus_trace_filter KCOVFUZZER=binfmt ./fuzzer -max_len=30 -only_ascii=1
// corpus_binfmt
//
// If you build with -static, then the following env needs to be exported:
// UBSAN_OPTIONS="handle_segv=0 handle_sigbus=0 handle_abort=0 handle_sigill=0
// handle_sigtrap=0 handle_sigfpe=0" and the following flags added to fuzzer
// invocation:
// -timeout=0 -rss_limit_mb=0 -handle_segv=0 -handle_bus=0 -handle_abrt=0
// -handle_ill=0 \ -handle_fpe=0 -handle_int=0 -handle_term=0 -handle_xfsz=0
...
void trace_filter(const char *data, long size) {
  int fd0 = open(
      "/sys/kernel/debug/tracing/events/syscalls/sys_exit_read/enable", O_RDWR);
  if (fd0 == -1)
    fail("open enable failed");
  int fd1 = open(
      "/sys/kernel/debug/tracing/events/syscalls/sys_exit_read/filter", O_RDWR);
  if (fd1 == -1)
    fail("open filter failed");
  int fd2 =
      open("/sys/kernel/debug/tracing/events/syscalls/sys_exit_read/trigger",
           O_RDWR);
  if (fd2 == -1)
...
void cover_start() { __atomic_store_n(&kcov_data[0], 0, __ATOMIC_RELAXED); }
