...
func MutateWithHints(ps []*Prog, subNum int, callIndex int, comps CompMap, exec func(ps []*Prog)) {
	ps = Clones(ps)
	c := ps[subNum].Calls[callIndex]
	execValidate := func() {
		...
		if ps[subNum].Target.sanitize(c, false) != nil {
			return
		}
		ps[subNum].debugValidate()
		exec(ps)
	}
