...
static void init_coverage_filter(char *filename) {
  ...
  if (f < 0) {
    // We don't fail here because we don't know yet if we should use coverage
    // filter or not. We will receive the flag only in execute flags and will
    // fail in coverage_filter if necessary.
  ...
  cov_filter =
      (cov_filter_t *)mmap(preferred, st.st_size, PROT_READ, MAP_PRIVATE, f, 0);
  if (cov_filter != preferred)
    failmsg("failed to mmap coverage filter bitmap", "want=%p, got=%p",
            preferred, cov_filter);
  if ((uint32)st.st_size !=
      sizeof(uint32) * 2 + ((cov_filter->pcsize >> 4) / 8 + 1))
...
static bool coverage_filter(uint64 pc) {
  ...
  if (pc32 < cov_filter->pcstart ||
      pc32 > cov_filter->pcstart + cov_filter->pcsize)
    return false;
