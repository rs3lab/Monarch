...
static zx_status_t update_exception_thread_regs(zx_handle_t exception) {
  ...
  status = zx_thread_read_state(thread, ZX_THREAD_STATE_GENERAL_REGS, &regs,
                                sizeof(regs));
  if (status != ZX_OK) {
...
#endif
    status = zx_thread_write_state(thread, ZX_THREAD_STATE_GENERAL_REGS, &regs,
                                   sizeof(regs));
    if (status != ZX_OK) {
...
static void *ex_handler(void *arg) {
  ...
  for (int i = 0; i < 10000; i++) {
    zx_status_t status = zx_object_wait_one(
        exception_channel, ZX_CHANNEL_READABLE, ZX_TIME_INFINITE, NULL);
    if (status != ZX_OK) {
    ...
    status = zx_channel_read(exception_channel, 0, &info, &exception,
                             sizeof(info), 1, NULL, NULL);
    if (status != ZX_OK) {
    ...
    status = zx_object_set_property(exception, ZX_PROP_EXCEPTION_STATE, &state,
                                    sizeof(state));
    if (status != ZX_OK) {
...
static void install_segv_handler(void) {
  ...
  if ((status = zx_task_create_exception_channel(zx_process_self(), 0,
                                                 &exception_channel)) != ZX_OK)
    failmsg("zx_task_create_exception_channel failed", "status=%s (%d)",
            zx_status_get_string(status), status);
  pthread_t th;
...
// Fuchsia's pthread_cond_timedwait just returns immidiately, so we use simple
// spin wait.
typedef struct {
  int state;
} event_t;
...
long syz_mmap(size_t addr, size_t size) {
  ...
  zx_info_vmar_t info;
  zx_status_t status =
      zx_object_get_info(root, ZX_INFO_VMAR, &info, sizeof(info), 0, 0);
  if (status != ZX_OK) {
    debug("zx_object_get_info(ZX_INFO_VMAR) failed: %s (%d)",
          zx_status_get_string(status), status);
    return status;
  }
  zx_handle_t vmo;
...
static long syz_process_self(void) { return zx_process_self(); }
#endif

#if SYZ_EXECUTOR || __NR_syz_thread_self
static long syz_thread_self(void) { return zx_thread_self(); }
#endif

#if SYZ_EXECUTOR || __NR_syz_vmar_root_self
static long syz_vmar_root_self(void) { return zx_vmar_root_self(); }
#endif

#if SYZ_EXECUTOR || __NR_syz_job_default
static long syz_job_default(void) { return zx_job_default(); }
#endif
