...
static int test_one(int text_type, const char *text, int text_size, int flags,
                    unsigned reason, bool check_rax) {
  ...
  struct kvm_run *cpu_mem = (struct kvm_run *)mmap(
      0, cpu_mem_size, PROT_READ | PROT_WRITE, MAP_SHARED, cpufd, 0);
  if (cpu_mem == MAP_FAILED) {
    printf("cpu mmap failed (%d)\n", errno);
    return 1;
  }
  int vm_mem_size =
      24 * SYZ_PAGE_SIZE; // Allocate what executor allocates for vma[24]
  void *vm_mem = mmap(0, vm_mem_size, PROT_READ | PROT_WRITE,
  ...
  if (syz_kvm_setup_cpu(vmfd, cpufd, (uintptr_t)vm_mem, (uintptr_t)&kvm_text, 1,
                        flags, 0, 0)) {
...
static int test_kvm() {
  ...
  // TODO: test VM mode.
  // const char text16_vm[] =
  // "\x48\xc7\xc3\xde\xc0\xad\x0b\x90\x90\x48\xc7\xc0\xef\xcd\xab\x00\xf4"; if
  // (res = test_one(64, text16_vm, sizeof(text16_vm) - 1, KVM_SETUP_VM,
  // KVM_EXIT_HLT, true))
  //	return res;

  // TODO: test code executed in interrupt handlers.
  // const char text32_div0[] = "\x31\xc0\xf7\xf0";
  // if (res = test_one(32, text32_div0, sizeof(text32_div0)-1, 0, KVM_EXIT_HLT,
  // true))
  //	return res;
  ...
  if ((res = test_one(8, text8, sizeof(text8) - 1, 0, KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(8, text8, sizeof(text8) - 1, KVM_SETUP_VIRT86,
                      KVM_EXIT_SHUTDOWN, true)))
    return res;
  if ((res = test_one(8, text8, sizeof(text8) - 1,
                      KVM_SETUP_VIRT86 | KVM_SETUP_PAGING, KVM_EXIT_SHUTDOWN,
                      true)))
    return res;
  ...
  if ((res = test_one(16, text16, sizeof(text16) - 1, 0, KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(16, text16, sizeof(text16) - 1, KVM_SETUP_CPL3,
                      KVM_EXIT_SHUTDOWN, true)))
    return res;
  ...
  if ((res = test_one(32, text32, sizeof(text32) - 1, 0, KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(32, text32, sizeof(text32) - 1, KVM_SETUP_PAGING,
                      KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(32, text32, sizeof(text32) - 1, KVM_SETUP_CPL3,
                      KVM_EXIT_SHUTDOWN, true)))
    return res;
  ...
  if ((res = test_one(64, text64, sizeof(text64) - 1, 0, KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(64, text64, sizeof(text64) - 1, KVM_SETUP_PAGING,
                      KVM_EXIT_HLT, true)))
    return res;
  if ((res = test_one(64, text64, sizeof(text64) - 1, KVM_SETUP_CPL3,
                      KVM_EXIT_SHUTDOWN, true)))
    return res;

  const char text64_sysenter[] = "\xb8\xde\xc0\xad\x0b\x0f\x34";
  if ((res = test_one(64, text64_sysenter, sizeof(text64_sysenter) - 1,
                      KVM_SETUP_CPL3, KVM_EXIT_SHUTDOWN, true)))
    return res;

  // Note: SMM does not work on 3.13 kernels.
  if (ver >= 404) {
    const char text8_smm[] = "\x66\xb8\xde\xc0\xad\x0b";
    if ((res = test_one(8, text8_smm, sizeof(text8_smm) - 1, KVM_SETUP_SMM,
                        KVM_EXIT_HLT, true)))
      return res;
    if ((res =
             test_one(8, text8_smm, sizeof(text8_smm) - 1,
                      KVM_SETUP_SMM | KVM_SETUP_PROTECTED, KVM_EXIT_HLT, true)))
      return res;

    // const char text32_smm[] = "\xb8\xde\xc0\xad\x0b";
    if ((res = test_one(32, text8_smm, sizeof(text8_smm) - 1, KVM_SETUP_SMM,
                        KVM_EXIT_HLT, true)))
      return res;

    // Also ensure that we are actually in SMM.
    // If we do MOV to RAX and then RSM, RAX will be restored to host value so
    // RAX check will fail. So instead we execute just RSM, if we are in SMM we
    // will get KVM_EXIT_HLT, otherwise KVM_EXIT_INTERNAL_ERROR.
    const char text_rsm[] = "\x0f\xaa";
    if ((res = test_one(8, text_rsm, sizeof(text_rsm) - 1, KVM_SETUP_SMM,
                        KVM_EXIT_HLT, false)))
      return res;
    if ((res = test_one(32, text_rsm, sizeof(text_rsm) - 1, KVM_SETUP_SMM,
                        KVM_EXIT_HLT, false)))
      return res;
  }
#elif GOARCH_ppc64le
  for (unsigned i = 0; i < (1 << 5); ++i) {
    res = test_one(8, kvm_ppc64_mr, sizeof(kvm_ppc64_mr) - 1, i, KVM_EXIT_DEBUG,
                   true);
    if (res)
      return res;
    res = test_one(8, kvm_ppc64_ld, sizeof(kvm_ppc64_ld) - 1, i, KVM_EXIT_DEBUG,
                   true);
    if (res)
      return res;
  }
...
static void dump_seg(const char *name, struct kvm_segment *seg) {
  printf("%s: base=0x%llx limit=0x%x sel=0x%x type=%d p=%d dpl=%d, db=%d s=%d "
         "l=%d g=%d\n",
         name, seg->base, seg->limit, seg->selector, seg->type, seg->present,
         seg->dpl, seg->db, seg->s, seg->l, seg->g);
}
#endif
...
static void dump_cpu_state(int cpufd, char *vm_mem) {
  ...
  printf("RIP=0x%llx RAX=0x%llx RDX=0x%llx RCX=0x%llx RBX=0x%llx CF=%d ZF=%d\n",
         regs.rip, regs.rax, regs.rdx, regs.rcx, regs.rbx,
         !!(regs.rflags & (1 << 0)), !!(regs.rflags & (1 << 6)));
  ...
  if (false) {
    ...
    for (int i = 0; i < vmcs_size / 8; i += 4) {
      printf("0x%04x: 0x%016llx 0x%016llx 0x%016llx 0x%016llx\n", i,
             ((long long *)vm_mem)[i], ((long long *)vm_mem)[i + 1],
             ((long long *)vm_mem)[i + 2], ((long long *)vm_mem)[i + 3]);
    }
  }
  ...
  printf("GPR08 %016lx %016lx %016lx %016lx\n", regs.gpr[8], regs.gpr[9],
         regs.gpr[10], regs.gpr[11]);
  printf("GPR12 %016lx %016lx %016lx %016lx\n", regs.gpr[12], regs.gpr[13],
         regs.gpr[14], regs.gpr[15]);
  printf("GPR16 %016lx %016lx %016lx %016lx\n", regs.gpr[16], regs.gpr[17],
         regs.gpr[18], regs.gpr[19]);
  printf("GPR20 %016lx %016lx %016lx %016lx\n", regs.gpr[20], regs.gpr[21],
         regs.gpr[22], regs.gpr[23]);
  printf("GPR24 %016lx %016lx %016lx %016lx\n", regs.gpr[24], regs.gpr[25],
         regs.gpr[26], regs.gpr[27]);
  printf("GPR28 %016lx %016lx %016lx %016lx\n", regs.gpr[28], regs.gpr[29],
         regs.gpr[30], regs.gpr[31]);
