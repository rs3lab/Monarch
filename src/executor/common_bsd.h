...
static void execute_command(bool panic, const char *format, ...) {
  ...
  // On some distributions, system/shell adds a minimal PATH, on some it does
  // not. Set own standard PATH to make it work across distributions.
...
static void initialize_tun(int tun_id) {
  ...
  execute_command(1, "ifconfig %s inet %s netmask 255.255.255.0", tun_iface,
                  local_ipv4);
...
#if !GOOS_darwin && SYZ_EXECUTOR ||                                            \
    __NR_syz_extract_tcp_res && SYZ_NET_INJECTION
...
static long syz_extract_tcp_res(volatile long a0, volatile long a1,
  ...
  if (ethhdr->ether_type == htons(ETHERTYPE_IP)) {
    ...
    if (iphdr->ip_p != IPPROTO_TCP)
      return (uintptr_t)-1;
    if (length <
        sizeof(struct ether_header) + iphdr->ip_hl * 4 + sizeof(struct tcphdr))
      return (uintptr_t)-1;
    tcphdr =
        (struct tcphdr *)&data[sizeof(struct ether_header) + iphdr->ip_hl * 4];
  } else {
    if (length < sizeof(struct ether_header) + sizeof(struct ip6_hdr))
      return (uintptr_t)-1;
    struct ip6_hdr *ipv6hdr =
        (struct ip6_hdr *)&data[sizeof(struct ether_header)];
    ...
    tcphdr = (struct tcphdr
                  *)&data[sizeof(struct ether_header) + sizeof(struct ip6_hdr)];
  }
