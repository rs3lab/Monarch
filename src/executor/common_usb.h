...
static bool parse_usb_descriptor(const char *buffer, size_t length,
  ...
  index->config =
      (struct usb_config_descriptor *)(buffer + sizeof(*index->dev));
...
// add_usb_index() and lookup_usb_index() helper functions allow to store and
// lookup per-device metadata associated with a file descriptor that is used to
// comminicate with a particular emulated device.
...
static bool analyze_control_request_standard(struct usb_device_index *index,
...
static bool analyze_control_request_class(struct usb_device_index *index,
                                          struct usb_ctrlrequest *ctrl) {
  uint8 bDeviceClass = index->bDeviceClass;
  uint8 bInterfaceClass = index->ifaces[index->iface_cur].bInterfaceClass;
  ...
  if (bDeviceClass == USB_CLASS_PRINTER ||
  ...
  if (bInterfaceClass == USB_CLASS_COMM) {
    ...
    case USB_CDC_SEND_ENCAPSULATED_COMMAND:
      debug(
          "analyze_control_request: req = USB_CDC_SEND_ENCAPSULATED_COMMAND\n");
      break;
    case USB_CDC_GET_ENCAPSULATED_RESPONSE:
      debug(
          "analyze_control_request: req = USB_CDC_GET_ENCAPSULATED_RESPONSE\n");
      break;
    ...
    case USB_CDC_REQ_SET_CONTROL_LINE_STATE:
      debug("analyze_control_request: req = "
...
static bool analyze_control_request_vendor(struct usb_device_index *index,
                                           struct usb_ctrlrequest *ctrl) {
  // Ignore vendor requests for now.
  return true;
}

// A helper function that prints a request in readable form and returns whether
// descriptions for this request exist. Needs to be updated manually when new
// descriptions are added. Useful for debugging.
static void analyze_control_request(int fd, struct usb_ctrlrequest *ctrl) {
  ...
  if (ctrl->bRequestType & USB_DIR_IN) {
    ...
    snprintf(&message[0], sizeof(message),
             "BUG: unknown control request (0x%x, 0x%x, 0x%x, 0x%x, %d)",
...
static const char default_string[] = {
    8, USB_DT_STRING, 's', 0, 'y', 0, 'z', 0 // UTF16-encoded "syz"
};

static const char default_lang_id[] = {
    4, USB_DT_STRING, 0x09, 0x04 // English (United States)
};

// lookup_connect_response_in() is a helper function that returns a response to
// a USB IN request based on syzkaller-generated arguments provided to
// syz_usb_connect* pseudo-syscalls. The data and its length to be used as a
// response are returned in *response_data and *response_length. The return
// value of this function lookup_connect_response_inindicates whether the
// request is known to syzkaller.
...
// lookup_connect_response_out() functions process a USB OUT request and return
// in *done whether this is the last request that must be handled by
// syz_usb_connect* pseudo-syscalls.
...
// lookup_control_response() is a helper function that returns a response to a
// USB IN request based on syzkaller-generated arguments provided to
// syz_usb_control_io* pseudo-syscalls. The data and its length to be used as a
// response are returned in *response_data and *response_length. The return
// value of this function indicates whether the response for this request is
// provided in syz_usb_control_io* arguments.
