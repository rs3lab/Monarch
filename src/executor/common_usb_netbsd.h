...
static volatile long syz_usb_connect_impl(
    int fd, uint64 speed, uint64 dev_len, const char *dev,
  ...
  while (!done) {
    ...
    debug("syz_usb_connect: bReqType: 0x%x (%s), bReq: 0x%x, wVal: 0x%x, wIdx: "
          "0x%x, wLen: %d\n",
          req.u.ctrl.bmRequestType,
          (req.u.ctrl.bmRequestType & UE_DIR_IN) ? "IN" : "OUT",
          req.u.ctrl.bRequest, UGETW(req.u.ctrl.wValue),
          UGETW(req.u.ctrl.wIndex), UGETW(req.u.ctrl.wLength));
