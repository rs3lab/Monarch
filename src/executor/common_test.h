...
// syz_compare(want ptr[in, string], want_len len[want], got ptr[in,
// compare_data], got_len len[got])
static long syz_compare(volatile long want, volatile long want_len,
                        volatile long got, volatile long got_len) {
  if (want_len != got_len) {
    errno = EBADF;
    goto error;
  }
