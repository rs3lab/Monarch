...
#define RADIX_PTE_INDEX_SIZE                                                   \
  5 // size: 8B <<  5 = 256B, maps 2^5  x   64K =   2MB
#define RADIX_PMD_INDEX_SIZE                                                   \
  9 // size: 8B <<  9 =  4KB, maps 2^9  x   2MB =   1GB
#define RADIX_PUD_INDEX_SIZE                                                   \
  9 // size: 8B <<  9 =  4KB, maps 2^9  x   1GB = 512GB
#define RADIX_PGD_INDEX_SIZE                                                   \
  13 // size: 8B << 13 = 64KB, maps 2^13 x 512GB =   4PB
...
static int kvmppc_define_rtas_kernel_token(int vmfd, unsigned token,
                                           const char *func) {
...
static int kvm_vm_enable_cap(int vmfd, uint32 capability, uint64 p1,
                             uint64 p2) {
  struct kvm_enable_cap cap = {
      .cap = capability,
      .flags = 0,
...
static void dump_text(const char *mem, unsigned start, unsigned cw,
                      uint32 debug_inst_opcode) {
#ifdef DEBUG
  printf("Text @%x: ", start);
...
// array[kvm_text, 1]], ntext len[text], flags flags[kvm_setup_flags_ppc64],
// opts ptr[in, array[kvm_setup_opt, 0:2]], nopt len[opts])
...
#define VEC(x) (*((uint32 *)(host_mem + (x))))
  ...
  memcpy(host_mem + BOOK3S_INTERRUPT_DECREMENTER, kvm_ppc64_recharge_dec,
         sizeof(kvm_ppc64_recharge_dec) - 1);
  VEC(BOOK3S_INTERRUPT_DECREMENTER + sizeof(kvm_ppc64_recharge_dec) - 1) =
      debug_inst_opcode;
  ...
  if (flags & (KVM_SETUP_PPC64_IR | KVM_SETUP_PPC64_DR)) {
    ...
    pgd[0] = cpu_to_be64(PPC_BIT(0) | // Valid
                         (pud_off & PPC_BITMASK(4, 55)) | RADIX_PUD_INDEX_SIZE);
    pud[0] = cpu_to_be64(PPC_BIT(0) | // Valid
                         (pmd_off & PPC_BITMASK(4, 55)) | RADIX_PMD_INDEX_SIZE);
    pmd[0] = cpu_to_be64(PPC_BIT(0) | // Valid
                         (pte_off & PPC_BITMASK(4, 55)) | RADIX_PTE_INDEX_SIZE);
    ...
    process_tb[0].prtb0 =
        cpu_to_be64(PATB_HR | rts1 | pgd_off | rts2 | RADIX_PGD_INDEX_SIZE);
    if (pid)
      process_tb[pid].prtb0 =
          cpu_to_be64(PATB_HR | rts1 | pgd_off | rts2 | RADIX_PGD_INDEX_SIZE);
    ...
    struct kvm_ppc_mmuv3_cfg cfg = {
        ...
        .process_table =
            (process_tb_off & PRTB_MASK) | (PRTB_SIZE_SHIFT - 12) | PATB_GR,
    };
    ...
    printf(
        ...
        "%016lx\n",
        pte_off, be64_to_cpu((unsigned long)pte[0]),
        be64_to_cpu((unsigned long)pte[1]), be64_to_cpu((unsigned long)pte[2]),
        be64_to_cpu((unsigned long)pte[3]), be64_to_cpu((unsigned long)pte[4]),
        be64_to_cpu((unsigned long)pte[5]), be64_to_cpu((unsigned long)pte[6]),
  ...
  if (!(flags & KVM_SETUP_PPC64_LE)) {
    ...
    for (unsigned long i = 0; i < sizeof(kvm_ppc64_recharge_dec) / sizeof(*p);
         ++i)
      p[i] = cpu_to_be32(p[i]);
  } else {
