...
static int usb_raw_init(int fd, uint32 speed, const char *driver,
                        const char *device) {
...
static int lookup_interface(int fd, uint8 bInterfaceNumber,
...
static volatile long syz_usb_connect_impl(
    uint64 speed, uint64 dev_len, const char *dev,
  ...
  // triggering interaction between multiple USB devices within the same
  // program.
  char device[32];
  ...
  while (!done) {
    ...
    debug("syz_usb_connect: bReqType: 0x%x (%s), bReq: 0x%x, wVal: 0x%x, wIdx: "
          "0x%x, wLen: %d\n",
          event.ctrl.bRequestType,
          (event.ctrl.bRequestType & USB_DIR_IN) ? "IN" : "OUT",
          event.ctrl.bRequest, event.ctrl.wValue, event.ctrl.wIndex,
          event.ctrl.wLength);
...
static volatile long syz_usb_control_io(volatile long a0, volatile long a1,
  ...
  debug("syz_usb_control_io: bReqType: 0x%x (%s), bReq: 0x%x, wVal: 0x%x, "
        "wIdx: 0x%x, wLen: %d\n",
        event.ctrl.bRequestType,
        (event.ctrl.bRequestType & USB_DIR_IN) ? "IN" : "OUT",
        event.ctrl.bRequest, event.ctrl.wValue, event.ctrl.wIndex,
