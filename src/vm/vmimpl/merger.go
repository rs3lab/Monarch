...
func (merger *OutputMerger) Add(name string, r io.ReadCloser, stop chan bool) {
	merger.AddDecoder(name, r, nil, stop)
}

func (merger *OutputMerger) AddDecoder(name string, r io.ReadCloser,
	decoder func(data []byte) (start, size int, decoded []byte), stop chan bool) {
	merger.wg.Add(1)
	go func() {
		var pending []byte
		var proto []byte
		var buf [4 << 10]byte
		for {
			//tao added for stop output collection after VM has been successuflly started.
			//After this, another new merger would be setup.
			select {
            case <- stop:
				close(stop)
				merger.wg.Done()
